...
export class PluginError extends Error {
    ...
    toJSON(): object {
        return {
            ...
            cause:
                this.cause instanceof PluginError
                    ? this.cause.toJSON()
...
export class DefaultPluginErrorHandler implements PluginErrorHandler {
    ...
    constructor(maxFailuresBeforeDisable: number = 3, logger: Console = console) {
...
export class PluginErrorBoundary {
    ...
    reset(): void {
        ...
        if (this.handler instanceof DefaultPluginErrorHandler) {
            ...
            typeof (this.handler as unknown as Record<string, unknown>).resetFailureCount ===
            "function"
        ) {
            ;(
                this.handler as unknown as { resetFailureCount(name: string): void }
            ).resetFailureCount(this.pluginName)
        }
    }
    ...
    getFailureCount(): number {
        ...
        if (
            typeof (this.handler as unknown as Record<string, unknown>).getFailureCount ===
            "function"
        ) {
            return (
                this.handler as unknown as { getFailureCount(name: string): number }
            ).getFailureCount(this.pluginName)
        }
        return 0
    }
}
 ...
 */
let globalErrorBoundaryFactory: PluginErrorBoundaryFactory = new PluginErrorBoundaryFactory()
