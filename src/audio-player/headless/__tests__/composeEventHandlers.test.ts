...
import { isSAPDefaultPrevented, composeEventHandlers } from "../composeEventHandlers"
...
describe("isSAPDefaultPrevented", () => {
    ...
    it("returns true when nativeEvent.sapPreventDefault is true", () => {
        expect(isSAPDefaultPrevented({ nativeEvent: { sapPreventDefault: true } })).toBe(true)
        expect(isSAPDefaultPrevented({ nativeEvent: { sapPreventDefault: false } })).toBe(false)
