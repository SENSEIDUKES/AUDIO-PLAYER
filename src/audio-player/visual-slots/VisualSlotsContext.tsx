...
export function VisualSlotsProvider({ children }: VisualSlotsProviderProps) {
    const [activeBySlot, setActiveBySlot] = useState<Record<string, string | null>>(seedActive)
    const [settingsById, setSettingsById] =
        useState<Record<string, Record<string, unknown>>>(seedSettings)
    const defaultsByIdRef = useRef<Record<string, Record<string, unknown>>>({})
    ...
    const getActive = useCallback((slot: VisualSlot) => activeBySlot[slot] ?? null, [activeBySlot])
    ...
    const getSettings = useCallback(
        (id: string): Record<string, unknown> => settingsById[id] ?? getProviderDefaults(id),
    ...
    return <VisualSlotsContext.Provider value={value}>{children}</VisualSlotsContext.Provider>
}
