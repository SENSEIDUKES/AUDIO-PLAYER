...
## 2024-05-24 - [Prevent DOM XSS by avoiding innerHTML for container clearing]
...
**Learning:** Using innerHTML = "" triggers the HTML parser and can be flagged by security scanners as a potential XSS vector, even when clearing an element. Using textContent = "" or element.replaceChildren() is safer, faster, and avoids parsing overhead.
**Prevention:** Always use textContent = "" or replaceChildren() instead of innerHTML = "" when clearing the contents of a DOM element to maintain a strong security posture and satisfy static analysis tools.
